...
def test_framer():
    ...
    # Ayatul Kursi (2:255)
    words_text = database_manager.fetch_all_words_from_verse(1, 2)
    ...
    word_wbw_images = []
    word_wbw_images.extend(annotate_word(word_images[index], 1, 2, index + 1) for index in range(len(word_images)))
    print("Arranging words into verses with stop-sign overflow...")
    word_wbw_images.append(verse_number(2, padding=(1, 71, 1, 1)))
    images = frame(word_wbw_images)

    output_dir = "./output/test/"
    os.makedirs(output_dir, exist_ok=True)

    save_path = f"{output_dir}/framer.png"
