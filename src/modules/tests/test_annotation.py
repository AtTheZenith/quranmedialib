...
def _test_set(surah, ayah, word_range):
    # Fetch all words
    verse_text = db.get_verse(surah, ayah)
    words = verse_text.split()
    word_images = [get_wimage(w) for w in words]
    ...
    annotated_images = annotate_words(sliced_images, surah, ayah, start=start_idx, end=end_idx)
...
def test_annotate_words():
    ...
    annotated_images = _test_set(1, 1, (1, 2))
    ...
    annotated_images = _test_set(11, 89, (10, 11))
    ...
    annotated_images = _test_set(11, 113, (10, 12))
    assert len(annotated_images) == 1, f"Expected 1 image due to batching, got {len(annotated_images)}"
