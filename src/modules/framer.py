...
def _fits_on_page(current_y: int, row_height: int, config: LayoutConfig) -> bool:
    ...
    # current_y + row_height must be <= image_height - bottom_padding - bottom_offset
    # available_height = image_height - top - bottom - offset
    # so target_limit = available_height + top
...
def frame(
    ...
    if word_config is None:
        word_config = WordConfig(word_spacing=20, row_spacing=30, max_rows_per_page=5)

    # Strict Validation
    all_items = list(words)

    # Check for missing images to prevent layout crashes
    ...
    while all_items:
        ...
        if translation_images and page_index < len(translation_images):
            if trans_img := translation_images[page_index]:
                # Vertical positioning relative to screen available height
                trans_y = config.padding[0] + config.timage_y_offset
                if config.timage_vertical_align == "center":
                    trans_y = config.padding[0] + (config.available_height - trans_img.height) // 2 + config.timage_y_offset
                ...
                # Horizontal positioning relative to screen available width
