...
def _group_items_into_rows(
    ...
    while current_pos < len(all_items) and len(page_rows) < word_config.max_rows_per_page:
        row_items, row_consumed, row_width, max_row_height = _build_row(all_items, current_pos, config, word_config)
        ...
        if not _fits_on_page(current_y, max_row_height, config):
            ...
            if not page_rows:
                page_rows.append((row_items, row_width, max_row_height))
...
def _apply_stop_sign_adjustment(
    current_image_rows: list[tuple[list[WordItem], int, int]],
    items_consumed: int,
    word_config: WordConfig,
    ...
    for row_index in range(len(current_image_rows) - 1, -1, -1):
        row = current_image_rows[row_index][0]
...
def _render_page(
    ...
    total_verse_height = sum(row[2] for row in rows) + (len(rows) - 1) * word_config.row_spacing if rows else 0

    draw_y = _get_verse_start_y(total_verse_height, config, word_config)

    # Local references and constants for performance
    ...
    right_anchor = config.max_width - config.padding.right

    for i, (row, row_width, max_row_height) in enumerate(rows):
        current_x = right_anchor

        if config.wimage_horizontal_align == HorizontalAlignment.CENTER:
            if row_width < config.content_width:
                current_x = config.padding.left + (config.content_width - row_width) // 2 + row_width
        elif config.wimage_horizontal_align == HorizontalAlignment.LEFT:
            if row_width < config.content_width:
                current_x = config.padding.left + row_width
        current_x += config.wimage_x_offset
...
def frame(
    ...
    while current_index < total_items:
        ...
        if any(it.text for it in all_items[current_index:]) and items_consumed < remaining_items:
            current_rows, items_consumed = _apply_stop_sign_adjustment(current_rows, items_consumed, word_config)
        ...
        if word_config.balanced_wrapping and len(current_rows) > 1:
            current_rows = _balance_image_rows(
                list(itertools.chain.from_iterable(r[0] for r in current_rows)),
                len(current_rows),
                config,
                word_config,
            )
