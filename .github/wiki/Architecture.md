...
The `framer.py` module takes these masks and arranges them into Right-to-Left (RTL) rows. It uses the **IPL-B (Inverted Pyramid Line Balancing)** algorithm to ensure that the text is visually centered and that lines decrease in width as they go down.
